...
At the prompt, type "python gambezi.py". This starts a text-based interface that accepts commands and eventually produces one or more scripts to install one or more applications on the Raspberry Pi. The top-level menu available on startup is the "build installer" menu. Certain commands may switch to a submenu with a different set of commands.

Use the "quit" command at the top-level to exit Gambezi. To return from a submenu, use the "cancel" command. Ctrl-C may be used at any time to abort the current run of Gambezi and return to the command prompt.
